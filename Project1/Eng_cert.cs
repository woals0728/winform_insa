...
namespace Project1
{
    public partial class Eng_cert : MetroForm
    {
        ...
        private void print_button_Click(object sender, EventArgs e)
        {
            if (dBManager.GetConnection() == true)
            {
                using (OracleCommand cmd = new OracleCommand())
                {
                    ...
                    try
                    {
                        cmd.CommandText = "select * from thrm_ceri_ljm where CERI_EMPNO = '" + empno + "' and CERI_KIND = '재직' and CERI_LANG = '영문' and CERI_DATE = '" + DateTime.Now.ToString("yyyyMMdd") + "'";
                        ...
                        if (reader.Read())
                        {
                            if (MessageBox.Show("이미 인쇄하신 내역이 있습니다." + Environment.NewLine + "재발급 하시겠습니까?", "", MessageBoxButtons.YesNo) == DialogResult.Yes)
                            {
                                cmd.CommandText = "update thrm_ceri_ljm set DATASYS1 = '" + DateTime.Now.ToString("yy/MM/dd") + "' where CERI_EMPNO = '" + empno + "' " +
                                    "and CERI_KIND = '재직' and CERI_LANG = '영문' and CERI_DATE = '" + DateTime.Now.ToString("yyyyMMdd") + "'";
                                cmd.ExecuteNonQuery();
                                tran.Commit();
                            }
                            else
                        ...
                        else
                        {
                            cmd.CommandText = "insert into thrm_ceri_ljm values('" + empno + "', '재직', (select max(ceri_no)+1 from thrm_ceri_ljm), '" + DateTime.Now.ToString("yyyyMMdd") + "'," +
                                "'영문', 1, '" + DateTime.Now.ToString("yy/MM/dd") + "', 'A', '" + user + "')";
                            cmd.ExecuteNonQuery();
                            tran.Commit();
                        }
                    }
