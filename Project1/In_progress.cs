...
namespace Project1
{
    public partial class In_progress : UserControl
    {
        ...
        private void In_progress_Load(object sender, EventArgs e)
        {
            DataLoad();
            ...
            try
            {
                if (dBManager.GetConnection() == true)
                {
                    using (OracleCommand cmd = new OracleCommand())
                    {
                        ...
                        cmd.CommandText = "select substr(lpad(bas_entdate, 10, '20'),1,4) as Year, " +
                            "substr(lpad(bas_entdate, 10, '20'),6,2) as Month, count(*) as Value " +
                            "from thrm_bas_ljm group by substr(lpad(bas_entdate, 10, '20'),1,4), substr(lpad(bas_entdate, 10, '20'),6,2)";
